...
def _lemke_howson_tbl(tableaux, bases, init_pivot, max_iter):
    ...
    while True:
        for pl in pls:
            ...
            gauss_jordan_pivoting(tableaux[pl], pivot, row_min)
...
def _lex_min_ratio_test(tableau, pivot, slack_start, argmins):
    ...
    num_argmins = min_ratio_test(tableau, pivot, -1, argmins, num_candidates)
    if num_argmins == 1:
        return argmins[0]
    ...
    for j in range(slack_start, slack_start+nrows):
        if j == pivot:
            continue
        num_argmins = min_ratio_test(tableau, pivot, j, argmins, num_argmins)
        if num_argmins == 1:
            break
    return argmins[0]
